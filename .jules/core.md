...
## 2026-03-06: Historical Scores Aggregation Logic Extraction

- **Logic Extracted:** Duplicated logic for aggregating a group's historical scores by year (with optional apparatus filtering) and sorting them chronologically. Found duplicated in `GroupInfoCard.vue` and `GroupDetailsModal.vue`.
- **Destination:** `app/utils/history.ts`
- **Changes:**
  - Created a pure utility function `aggregateHistoryByYear(history, apparatusCode?)`.
  - Defined strict TypeScript types `HistoryPoint`.
  - Replaced the inline mapped loops and manual sorting logic in both components with a call to the new utility.
- **Outcome:** Removed ~30 lines of duplicated array manipulation logic from components. Ensured that any future charts or historical analysis share the exact same logic and typing.
