...
- **Logic Extracted:** Duplicated logic for aggregating historical scores by year and calculating the overall average score from `GroupDetailsModal.vue` and `GroupInfoCard.vue`.
- **Destination:** `app/utils/history.ts`
- **Changes:**
  - Created `aggregateHistoryByYear()` pure function to compute total counts and calculate average scores grouped by year, with an optional apparatus filter.
  - Created `calculateAverageScore()` pure function to derive a formatted global average from an array of history items.
  - Replaced the duplicate logic in `historyByYear` and `averageScore`/`averageHistoryScore` computed properties within `GroupDetailsModal.vue` and `GroupInfoCard.vue`.
- **Outcome:** Removed ~30 lines of duplicated logic. Centralized pure business logic related to history statistics calculations outside of reactive component setups.
