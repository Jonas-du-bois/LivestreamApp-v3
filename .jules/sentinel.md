...
## 2026-03-05 - Unhandled CastError on Invalid ObjectId in API Endpoints
**Vulnerability:** The API endpoint `LiveStreamApp/server/api/streams/[id].get.ts` retrieved a stream by ID using `StreamModel.findById(id)` without validating the format of the `id` string passed via the router parameter. If a malformed ID (not a 24-character hex string) was provided, Mongoose would throw an unhandled `CastError`, resulting in a `500 Internal Server Error`. This represents an information disclosure and minor DoS vector as the server leaks stack details implicitly or consumes error handling resources unnecessarily.
**Learning:** Always validate that strings intended for MongoDB ObjectIds are structurally valid *before* passing them to database driver functions. Relying on the driver's implicit casting for input validation is a poor security pattern that leads to unhandled exceptions.
**Prevention:** Use `mongoose.Types.ObjectId.isValid(id)` (or a Zod regex check like `/^[0-9a-fA-F]{24}$/`) on all user-controlled input that will be used as a database ID, returning a structured `400 Bad Request` if the validation fails.
