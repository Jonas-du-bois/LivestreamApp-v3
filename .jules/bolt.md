...
## 2024-05-21 - Frontend Reactivity Performance (Vue Composables)
**Learning:** Inside Vue composables, especially inside high-frequency computed properties that map over large arrays (`passages`), repeatedly instantiating `new Date()` or repeatedly calling `.toLocaleDateString()` triggers excessive `Intl.DateTimeFormat` object allocation. This causes severe main thread CPU spikes and garbage collection pauses.
**Action:** When formatting dates or calculating timestamps inside loops (like in `usePassageFilters` or `useTranslatedData`), initialize a single `Date` object and a single `Intl.DateTimeFormat` instance outside the loop. Use `dateObj.setTime()` for timestamp calculations and `formatter.format()` to avoid redundant object instantiation.
