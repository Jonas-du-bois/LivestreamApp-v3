...
## 2026-06-19 - Optimize Nitro cache clearing
**Learning:** When clearing all entries from a Nitro `useStorage` cache, fetching all keys and mapping over them with `Promise.all` and `removeItem` creates unnecessary overhead and potential main thread spikes.
**Action:** Always prefer using the built-in `await cacheStorage.clear()` method.
