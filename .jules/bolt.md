...
## 2026-07-08 - Exclusive Projection in Mongoose Queries
**Learning:** Using inclusive projections to exclude large arrays from Mongoose queries creates regression risks by unintentionally dropping fields required by other endpoints or backend logic (like ranks calculation).
**Action:** Always use exclusive projections (e.g., `.select('-history -monitors')`) on root `.find()` or `.findById()` queries to explicitly drop heavy unbounded arrays without stripping required, unlisted fields.
