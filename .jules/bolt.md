...
## 2026-06-23 - Optimize Intl.DateTimeFormat caching
**Learning:** In performance-critical reactive computed properties mapping over large arrays, avoid instantiating multiple `Date` objects or `Intl.DateTimeFormat` instances per iteration. Passing `NaN` directly to `Intl.DateTimeFormat.prototype.format` throws a fatal `RangeError`.
**Action:** Cache `Intl.DateTimeFormat` outside the loop or in module scope, use `Date.parse()` to extract timestamps safely, and verify parsed timestamps with `Number.isNaN()` before formatting.
