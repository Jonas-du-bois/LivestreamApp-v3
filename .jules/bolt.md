...
## 2026-06-14 - Optimize Nitro cache clearing
**Learning:** Mapping over `cacheStorage.getKeys()` with `Promise.all` and `cacheStorage.removeItem()` creates unnecessary promise allocations and main thread overhead when clearing the entire cache.
**Action:** Use the built-in `await cacheStorage.clear()` method provided by unstorage when clearing all keys to improve performance and API resilience.
