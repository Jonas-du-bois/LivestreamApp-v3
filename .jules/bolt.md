...
## 2026-04-27 - Mongoose Batch Query Array Accumulation
**Learning:** Performing multiple independent `findByIdAndDelete()` operations inside a notification loop scales poorly when a large chunk of push subscriptions fail at once. By accumulating the expired IDs into an array and executing a single `SubscriptionModel.deleteMany({ _id: { $in: expiredSubscriptions } })` query, we eliminate the N+1 query overhead.
**Action:** Always refactor iterative Mongoose deletion or update loops to collect IDs in an array and run a single bulk `deleteMany` or `updateMany` operation instead.
