...
\n## 2026-03-10 - Database Native Ranking Calculation\n**Learning:** Computing ranks by fetching an array of all IDs into memory (`.find().sort().select().lean()`) and doing a linear search (`findIndex`) causes O(N) memory scaling and handles ties inconsistently based on MongoDB's default secondary sort order.\n**Action:** For numeric ranking calculations, use `.countDocuments()` with inequality operators (e.g., `score: { $gt: value }`). This correctly implements competition ranking, shifts the computational load to database indexes, and maintains O(1) application memory usage.
