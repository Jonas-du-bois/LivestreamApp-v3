...
## 2026-05-04 - Optimize Mongoose Queries with .lean() and Projection
**Learning:** For read-only API endpoints fetching related documents, `Mongoose.findById()` paired with deeply nested `populate()` causes high memory usage and serialization delays. This happens because Mongoose creates heavy Document instances for both the root and all populated fields.
**Action:** Always append `.lean()` to Mongoose queries in GET endpoints, and apply explicit `select` projections inside `.populate()` (e.g., `select: 'name code icon'`) to minimize database payload size.
