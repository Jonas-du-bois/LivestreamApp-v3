...
## 2026-06-22 - Nitro Unstorage Cache Clearing Bottleneck
**Learning:** Fetching all keys from Nitro's `useStorage` and using `Promise.all` to iterate over them and map to `removeItem` causes severe O(N) overhead and blocks the main thread with unnecessary promise allocations.
**Action:** Always prefer using the native `await cacheStorage.clear()` method to efficiently purge the unstorage cache without incurring iterative lookup penalties.
