...
## 2026-06-06 - Socket.io Payload Optimization via Mongoose Projection
**Learning:** Mongoose `.populate()` calls whose results are directly emitted as Socket.io payloads (e.g., in score updates) can accidentally transmit massive nested arrays (like `history`) if no field selection is provided, increasing JSON size and network traffic.
**Action:** Always use inclusive field projections (e.g., `.populate('group', 'name category')`) when populating relations for Socket.io events.
