...
## $(date +%Y-%m-%d) - Replace Mongoose Array Processing with countDocuments
**Learning:** For calculating entity ranks (like a leaderboard), fetching the entire target collection into Node.js memory (`PassageModel.find().lean().exec()`) and doing a linear `.findIndex()` creates a massive O(N) memory and processing bottleneck as the collection scales.
**Action:** Always prefer pushing aggregations and sorting to the database level. For ranking specifically, `Model.countDocuments({ score: { $gt: targetScore } }) + 1` performs the operation entirely within the DB engine, returning a single integer (O(1) memory footprint) while naturally implementing standard competition ranking for ties.
