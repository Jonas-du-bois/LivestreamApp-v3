...
## 2026-04-10 - Mongoose existence checks
**Learning:** Replaced `.findOne()` with `.exists()` for simple database presence checks to prevent fetching large documents and improve backend performance.
**Action:** Use `.exists()` instead of `.findOne()` when only checking for existence in Mongoose.
