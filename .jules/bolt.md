...
## 2024-04-10 - Server-side projection for Mongoose query iterations in scheduled tasks
**Learning:** Mongoose `.find()` without projection loads the entire document into memory. When querying `PassageModel` in a scheduled task (like `scheduler.ts`) where only a few fields (e.g. `_id`, `location`) are needed to update other models (`StreamModel`), omitting projection leads to unnecessary memory usage, especially for models with unbounded arrays. Also, removing `.populate()` where only IDs are needed for subsequent queries significantly improves performance and memory usage.
**Action:** Always append `.select('field1 field2')` to Mongoose `.find()` queries in background tasks where only specific fields are needed. Ensure that any `.populate()` calls are actually required by the downstream logic inside the loop; if only the reference `_id` is needed or the populated fields are not accessed, remove the `.populate()`.
