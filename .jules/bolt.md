...
## 2026-06-05 - Optimize Date Formatting with Cached Intl.DateTimeFormat
**Learning:** Repeatedly instantiating `Date` objects and calling `toLocaleDateString()` inside reactive loops or mapped computed properties (like in `useTranslatedData.ts` which formats dates for `SchedulePassageCard`) causes severe garbage collection (GC) pressure and main thread spikes, especially during large list rendering.
**Action:** In Vue Nuxt applications, create a module-scoped singleton cache (`Map`) for `Intl.DateTimeFormat` instances outside the composable. Use a helper to extract timestamps efficiently (`typeof val === 'string' ? Date.parse(val) : val.getTime ? val.getTime() : new Date(val).getTime()`) and always safeguard against `NaN` (from invalid string parsing) before passing the value to the cached formatter, as it will throw a fatal `RangeError`.
