...
## 2026-05-28 - Mongoose Exists vs FindOne for simple existence checks
**Learning:** In Mongoose, replacing `.findOne()` with `.exists()` for simple existence checks prevents the database driver from fetching and hydrating full document properties into memory, reducing overhead, especially in high-frequency schedulers.
**Action:** Always audit `.findOne()` calls. If only checking for existence and the document data isn't needed, replace it with `.exists()`.
