...
## 2026-04-12 - Mongoose Exists and Compound Indexes
**Learning:** In Mongoose, using `.findOne()` just to check existence fetches the full document. For high-frequency background tasks (like a scheduler polling every interval), this causes unnecessary memory allocation. Additionally, querying time thresholds (`$lte`) without compound indexes combined with status leads to collection scans.
**Action:** Replace `.findOne()` with `.exists()` for simple presence checks. Always add compound indexes (e.g., `{ status: 1, timeField: 1 }`) for fields frequently polled in schedulers.
