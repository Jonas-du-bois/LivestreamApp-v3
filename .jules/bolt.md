...
## 2026-06-15 - Cached Intl.DateTimeFormat
**Learning:** Repeatedly calling `new Date().toLocaleDateString()` in loops or computed properties creates multiple `Date` and `Intl.DateTimeFormat` instances, leading to main thread spikes and GC pressure.
**Action:** Cache `Intl.DateTimeFormat` instances using a singleton map outside the composable, and parse input strings into timestamps to format efficiently with `formatter.format(timestamp)`.
