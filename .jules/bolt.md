...
## $(date +%Y-%m-%d) - O(N) Array Operations to O(1) Database Queries
**Learning:** For calculating item ranks based on scores within a database collection, fetching the entire collection into memory with `.find().sort()` and using `.findIndex()` creates an O(N) memory bottleneck and transfers excessive data over the network.
**Action:** Replace memory-heavy array sorting and searching with optimized database-level queries like `Model.countDocuments({ score: { $gt: targetScore } }) + 1`. This reduces application memory complexity to O(1) and correctly implements standard competition ranking (where tied participants receive the same rank).
