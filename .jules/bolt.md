...
## 2026-04-10 - Direct Ranking via MongoDB Count
**Learning:** Calculating a rank by loading all documents into memory using `.find().sort().select().lean()` and performing an array `findIndex` results in O(N) memory scaling and is extremely inefficient as the dataset grows. Additionally, it handles ties inconsistently.
**Action:** Always use `.countDocuments()` directly on the database with an inequality operator (`{ score: { $gt: targetScore } }`) and add 1. This offloads calculation completely to the database engine and provides stable competition rankings for ties.