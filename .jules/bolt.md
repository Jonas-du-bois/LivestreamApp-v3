...
## 2026-04-12 - Memory-efficient Ranking with countDocuments
**Learning:** Calculating relative ranks by fetching full collections into memory (e.g., `.find().sort()`) and finding the index causes O(N) memory bottlenecks and slow performance, especially in high-traffic admin APIs.
**Action:** Always use database-level operations like `Model.countDocuments({ score: { $gt: targetScore } }) + 1` for standard competition ranking to eliminate in-memory array allocation and leverage DB indexes.
