...
## 2026-07-01 - Optimize Nitro Cache Clearing
**Learning:** Using `await cacheStorage.getKeys()` followed by a `Promise.all(keys.map(removeItem))` creates unnecessary main thread overhead and object allocations when invalidating the entire cache in Nitro.
**Action:** Always prefer the built-in `await cacheStorage.clear()` method from unstorage when clearing all cache entries to minimize latency and CPU spikes.
