...
## 2026-06-01 - Optimize Mongoose background queries
**Learning:** In high-frequency scheduled tasks (like `setInterval`), querying Mongoose with `findOne` to check for existence causes full Document instantiation, resulting in unnecessary memory allocation. Furthermore, polling queries using `$lte` on dates without appropriate compound indexes cause excessive collection scans.
**Action:** Always replace `.findOne()` with `.exists()` for simple existence checks to avoid fetching full documents. For high-frequency schedulers polling date thresholds (e.g., `$lte`), always add compound indexes (e.g., `{ status: 1, timeField: 1 }`) to the schema to prevent full collection scans.
