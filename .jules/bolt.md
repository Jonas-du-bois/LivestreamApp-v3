...
## 2026-04-19 - Rank Calculation Memory Optimization
**Learning:** Fetching an entire collection of scores into memory using `.find().sort()` and `.findIndex()` to calculate a passage's rank creates an O(N) memory bottleneck, which can severely impact performance during competition events with high data volumes. Standard competition ranking inherently defines rank as 'number of competitors scoring higher + 1'.
**Action:** Always use database-level operations like `Model.countDocuments({ score: { $gt: targetScore } }) + 1` instead of in-memory array manipulation for rankings. This delegates the counting workload to MongoDB, reducing application memory pressure to O(1) and inherently handling ties appropriately.
