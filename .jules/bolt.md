...
## 2026-05-07 - Optimizing Existence Checks and Date Threshold Polling
**Learning:** In high-frequency background polling (e.g. schedulers), using `findOne()` instead of `exists()` wastes memory by fetching full documents when only existence needs to be verified. Additionally, range queries like `$lte` on time fields combined with status checks perform collection scans unless targeted compound indexes exist.
**Action:** Always prefer `.exists()` over `.findOne()` when the document data isn't needed. Add compound indexes like `{ status: 1, timeField: 1 }` to schemas for scheduled threshold queries to minimize database load.
