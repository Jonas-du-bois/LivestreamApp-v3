...
## 2026-07-05 - Server-side projection for Mongoose find queries
**Learning:** Even when populating fields, Mongoose `.find()` will fetch the entire document from the database if no field selection is provided on the base query. For heavily relational models like `Passage` with unbounded arrays (e.g., `history`, `monitors`), this causes excessive memory usage and slower serialization.
**Action:** Always apply explicit `.select()` projections directly to Mongoose `.find()` and `.findById()` queries, not just their nested `.populate()` clauses.
