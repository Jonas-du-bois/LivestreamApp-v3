...
## 2026-06-09 - Mongoose .exists() Optimization & Scheduler Polling Indexes
**Learning:** Using `.findOne()` for simple truthiness checks fetches the full document from MongoDB, which wastes network bandwidth, memory, and CPU for deserialization. Additionally, high-frequency schedulers polling for date thresholds (``) without covering indexes cause excessive full collection scans.
**Action:** Always replace `.findOne()` with `.exists()` for boolean existence checks. Always add compound indexes (e.g., `{ status: 1, timeField: 1 }`) for fields heavily used in polling schedulers.
