...
## 2026-05-28 - Optimize scheduler queries using exists and indexes
**Learning:** Using `.findOne()` for simple existence checks fetches full documents unnecessarily. High-frequency schedulers polling date thresholds (like `$lte`) without compound indexes on the polled fields cause excessive collection scans.
**Action:** Replace `.findOne()` with `.exists()` for existence checks. Always add compound indexes (e.g., `{ status: 1, timeField: 1 }`) for fields queried heavily by schedulers.
