...
## $(date +%Y-%m-%d) - Optimize Server-Side Mongoose Projections
**Learning:** Default Mongoose `populate()` without projection options loads all fields of the referenced documents into memory. When referenced documents have massive, unbounded arrays (e.g. `history` on Group or Passage), omitting projections can trigger severe memory spikes and latency degradation, especially during background processing loops or highly concurrent API calls.
**Action:** Always explicitly declare `$select` properties or positional selection arguments (`.populate('ref', 'field1 field2')`) on Mongoose populations to restrict queries strictly to required fields, preventing unnecessary loading of heavy embedded sub-documents.
