...
## $(date +%Y-%m-%d) - Optimize backend queries by projecting necessary fields and using `.exists()` for truthiness checks
**Learning:** For scheduled database tasks pulling from large datasets, it is extremely beneficial to limit the amount of memory allocated on the main thread and database I/O by utilizing exclusionary projections (e.g., `.select('_id location')`) when only a subset of fields is strictly necessary for processing. Additionally, using `.exists()` over `.findOne()` avoids hydrating a full Mongoose Document when merely verifying existence.
**Action:** When working on backend tasks processing batches of Mongoose documents, carefully inspect all properties read downstream and implement exact field projections for `.find()` calls. Use `.exists()` whenever only a truthiness check (`if (!document)`) is performed.
