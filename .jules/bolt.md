...
## 2024-04-10 - O(N) In-Memory Rank Calculation
**Learning:** Fetching an entire collection into memory with `.find().sort().select('_id')` just to find an element's rank using `.findIndex()` creates an O(N) memory and compute bottleneck on the Node process, especially as the competition progresses and more passages are scored.
**Action:** Replace in-memory ranking with database-level operations: `countDocuments({ score: { $gt: targetScore } }) + 1`. This naturally implements standard competition ranking (tied scores share the rank) and shifts the heavy lifting to MongoDB.
