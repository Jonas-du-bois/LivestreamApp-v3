...
## 2026-05-24 - Replace Mongoose findOne with exists
**Learning:** In Mongoose, using .findOne() for simple existence checks fetches full documents, which is inefficient. Using .exists() is faster as it only checks for the presence of a document without returning it.
**Action:** Always use .exists() instead of .findOne() when only checking if a document is present, especially in frequently executed code like auth sessions or cron jobs.
