...
## 2026-06-19 - Server-side projection for Mongoose populate queries
**Learning:** Mongoose `populate()` fetches the entire document if no field selection is provided. `Group` and `Passage` documents have unbounded `history` arrays and `monitors` arrays. Fetching these on endpoints like `streams/[id]` or in `score-update` causes excessive memory usage, increased DB payload size, and slower serialization.
**Action:** Mongoose `.populate()` calls on heavily relational models must include explicit field projections (e.g., `.populate('group', 'name')`) to strictly specify which fields should be returned.
