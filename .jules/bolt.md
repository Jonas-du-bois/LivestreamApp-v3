...
## 2026-04-26 - Vue Reactivity Object Re-mapping
**Learning:** Re-mapping an entire reactive Record/object structure in Vue 3 via destructuring `resultsMap.value = { ...resultsMap.value, [key]: updatedList }` triggered excessive full-component re-renders.
**Action:** Use in-place updates coupled with array shallow copying `resultsMap.value[key] = [...list]` to precisely target nested reactive keys without cloning the entire parent structure.
