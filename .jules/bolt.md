...
## 2026-05-26 - Optimize O(N) template lookups in Vue
**Learning:** Calling array `.find()` inside Vue template methods that are evaluated during list rendering causes O(N*M) performance bottlenecks and unnecessary CPU spikes on re-renders.
**Action:** Replace frequent template array lookups with O(1) computed `Record<string, T>` maps. Ensure the computed map logic uses a loop that only assigns the first match (e.g. `if (!map[key])`) to perfectly mirror `.find()` behavior.
