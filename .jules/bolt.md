...
## 2026-05-15 - Scheduler Existence Check & Polling Optimization
**Learning:** Using `findOne()` for existence checks fetches entire documents unnecessarily. Additionally, high-frequency schedulers querying `status` and date boundaries (`$lte`) without compound indexes cause full collection scans.
**Action:** Always use `.exists()` when only checking for existence, and add compound indexes for scheduler polling patterns like `{ status: 1, timeField: 1 }`.
