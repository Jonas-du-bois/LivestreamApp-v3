...
## 2026-06-28 - Optimize Mongoose base query memory allocation
**Learning:** Applying `.select()` only to `.populate()` clauses does not prevent Mongoose from fetching the entire base document into memory first. For heavily relational models like `Passage`, this loads all fields before projection occurs, creating significant overhead.
**Action:** Always apply explicit `.select()` projections directly to the base `.find()` or `.findById()` queries to minimize the initial payload fetched from the database.
