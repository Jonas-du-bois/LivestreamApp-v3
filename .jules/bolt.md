...
## 2026-04-05 - Memory-Efficient Rank Calculation
**Learning:** Calculating an item's rank by fetching the entire collection into memory using `.find().sort()` and then `.findIndex()` creates an O(N) memory bottleneck and degrades performance as data grows.
**Action:** Use database-level operations like `Model.countDocuments({ score: { $gt: targetScore } }) + 1` to calculate the rank efficiently without loading unnecessary documents into memory. This also naturally implements standard competition ranking where tied participants share the same rank.
