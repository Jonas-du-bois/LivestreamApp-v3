...
## 2026-05-05 - Scheduler Query Optimization
**Learning:** High-frequency polling schedulers (e.g., running every 30s-60s) looking for date thresholds (`$lte`) without compound indexes and projection cause excessive collection scans and memory bloat. Replacing `.populate()` with explicit `.select()` and adding `{ status: 1, timeField: 1 }` indexes reduces DB CPU and main thread load.
**Action:** Always add compound indexes for time-based status transitions and use exclusionary/explicit `.select()` in background tasks instead of reusing UI-oriented `.populate()` queries.
