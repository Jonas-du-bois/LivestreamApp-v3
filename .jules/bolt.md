...
## 2026-04-15 - Rank Calculation Optimization
**Learning:** For calculating item ranks, using database-level operations like `Model.countDocuments({ score: { $gt: targetScore } })` instead of fetching full collections into memory with `.find().sort()` and using `.findIndex()` eliminates O(N) memory bottlenecks and reduces DB payload.
**Action:** Always use `countDocuments` for standard competition ranking where tied participants share the same rank.
