...
## 2026-06-29 - Fatal RangeError with Cached Intl Formats
**Learning:** When migrating from `new Date(value).toLocaleDateString()` to a cached `Intl.DateTimeFormat.prototype.format(value)` to optimize allocations, be aware that `Date.parse('invalid')` returns `NaN`. While `new Date('invalid')` safely handles formatting by returning 'Invalid Date', passing `NaN` directly to `Intl.DateTimeFormat.prototype.format(NaN)` throws a fatal `RangeError`.
**Action:** Always verify parsed timestamps with `Number.isNaN()` or use strict `typeof` checks before passing them to a cached formatter.
