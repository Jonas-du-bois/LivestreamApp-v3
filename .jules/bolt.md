...
## 2026-03-31 - Optimize stream fetch with lean() and field selection
**Learning:** Fetching stream data with deep populates (current passage, group, and apparatus) without `.lean()` and without field selection causes excessive memory overhead and bloated JSON responses due to Mongoose document hydration and inclusion of large unused fields (like `history` arrays).
**Action:** Always add `.lean()` and `select` specific fields when populating deep relationships in individual `GET` requests (e.g., `streams/[id].get.ts`) to minimize memory allocation and response sizes.
