...
## 2026-06-10 - Optimize scheduler document existence and polling
**Learning:** In high-frequency schedulers, using `.findOne()` just to check existence fetches the full document unnecessarily, wasting memory. Furthermore, polling fields like `startTime` or `endTime` with a `status` filter without compound indexes causes excessive collection scans.
**Action:** Replace `.findOne()` with `.exists()` for simple existence checks. Always add compound indexes (e.g., `{ status: 1, timeField: 1 }`) to schemas used in polling loops.
