...
## 2026-06-13 - Consistent Field Projections across Emitters
**Learning:** When optimizing Mongoose field projections to reduce payload size, missing fields required by frontend interfaces or backend computations (like `subCategory` for ranks) causes state inconsistencies and broken UI between REST endpoints and Socket.io emitters.
**Action:** Ensure the selected fields remain consistent across related REST endpoints (initial load) and Socket.io emitters (real-time updates) when applying field projections to Mongoose `.populate()` calls.
