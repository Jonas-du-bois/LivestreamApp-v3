...
## 2026-04-05 - Numeric Ranking Calculation Optimization
**Learning:** Using `.find().sort().findIndex()` for calculating rank causes O(N) memory scaling, as it fetches all documents into memory and requires application-side processing. This can create a significant bottleneck as the dataset grows.
**Action:** Always use `.countDocuments()` with inequality operators (e.g., `score: { $gt: value }`) to calculate rank directly via database indexes. Remember to handle edge cases appropriately (e.g., unpublished items or undefined values).
