...
## 2026-06-11 - Server-side projection for Mongoose populate queries missed cases
**Learning:** Mongoose `populate()` without field projections was still present in `score-update.ts` and `streams/[id].get.ts`. Fetching the entire `Group` and `Apparatus` documents without projection pulls unbounded arrays into memory.
**Action:** Mongoose `.populate()` calls on relational models must include explicit field projections. Replaced `.populate('group')` with explicit projections like `.populate('group', 'name category subCategory canton logo')` and used `select` within nested populate configurations.
