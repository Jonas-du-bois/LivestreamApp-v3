...
## 2026-03-30 - Extract Query-Independent Aggregation
**Learning:** Query-independent aggregations inside Nitro API endpoints (like `results.get.ts`) that use `defineCachedEventHandler` still execute the aggregation every time there is a cache miss on the main route (e.g., due to different filter parameters). This introduces redundant database load.
**Action:** Extract query-independent metadata fetches (e.g., "available days") into separate functions wrapped with `defineCachedFunction` and specific TTLs, so the main route can await the globally cached result.
