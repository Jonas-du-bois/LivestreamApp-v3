...
## 2026-05-18 - Avoid multi-allocations of Date objects in reactive arrays\n**Learning:** Mapping over large reactive arrays allocating `new Date(string)` multiple times per iteration causes memory bloat and garbage collection pressure, reducing performance when array recomputes. Using `Date.parse(string)` avoids new Date allocation, and reusing a single  instance with `setTime` prevents further allocations.\n**Action:** Audit large array mapping functions (like sorting or time tracking on passages) to use `Date.parse` instead of `new Date()`, and reuse shared `Date` instances for time/date logic.
## 2026-05-18 - Avoid multi-allocations of Date objects in reactive arrays
**Learning:** Mapping over large reactive arrays allocating `new Date(string)` multiple times per iteration causes memory bloat and garbage collection pressure, reducing performance when array recomputes. Using `Date.parse(string)` avoids new Date allocation, and reusing a single Date instance with `setTime` prevents further allocations.
**Action:** Audit large array mapping functions (like sorting or time tracking on passages) to use `Date.parse` instead of `new Date()`, and reuse shared Date instances for time/date logic.
