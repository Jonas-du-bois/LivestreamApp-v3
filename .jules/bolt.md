...
## 2026-05-31 - Backend Performance: Mongoose Queries in Schedulers
**Learning:** Using `findOne()` for simple existence checks fetches full documents unnecessarily. High-frequency schedulers polling date thresholds (like `$lte`) require compound indexes (e.g., `{ status: 1, timeField: 1 }`) to prevent excessive collection scans.
**Action:** Replace `findOne()` with `exists()` for simple checks and always add compound indexes for frequently polled date thresholds.
