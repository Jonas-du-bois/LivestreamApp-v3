...
## 2026-06-04 - Optimize Intl.DateTimeFormat and Date object allocations
**Learning:** Calling `toLocaleDateString()` or instantiating new `Date` objects inside array loops (like Vue computed properties handling filtering) causes significant CPU spikes and GC pressure.
**Action:** Cache `Intl.DateTimeFormat` instances in module scope and use a lightweight `extractTimestamp` helper (`typeof val === 'string' ? Date.parse(val) : val.getTime ? val.getTime() : new Date(val).getTime()`) to pass primitive timestamps directly to `.format()`.

## 2026-06-04 - Handle invalid dates when caching Intl.DateTimeFormat
**Learning:** While `new Date('invalid').toLocaleDateString()` gracefully returns the string "Invalid Date", using a cached `Intl.DateTimeFormat` instance and calling `.format(NaN)` throws a fatal `RangeError`.
**Action:** When extracting primitive timestamps to optimize allocations, always verify the result with `Number.isNaN()` and manually return "Invalid Date" to prevent app crashes from bad data.
