...
## 2026-02-21: Stream Pages & Domain Components Strict Typing & SSR Safety\n\n- **Target:** `app/pages/stream/index.vue`, `app/pages/stream/[id].vue`, `app/components/domain/StreamCard.vue`, `app/pages/afterparty.vue`\n- **Risks Mitigated:**\n    - **SSR Hydration Mismatch:** Replaced explicit client-side fetching via `fetchLive`/`fetchStreams` in `onMounted` with Nuxt's SSR-safe composable-backed methods (`getLive`, `getStream`) using `useApiClient` which leverages `useFetch`/`useAsyncData`.\n    - **Code Fragility & Redundancy:** Removed unnecessary manual explicit imports of core Vue functionality like `computed` and `resolveComponent` to align with Nuxt auto-import best practices.\n- **Solution:**\n    - Added `getStream` method in `app/services/public.service.ts`.\n    - Refactored components to use SSR fetchers returning reactive `data`, `pending`, and `refresh`.\n    - Replaced `onMounted` data hydration with top-level `await` for initial fetching combined with a `handleRefresh` mechanism leveraging `refreshLive()` and `refreshStreams()`.
