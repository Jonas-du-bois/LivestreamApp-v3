...
## 2026-06-08 - Tactile Feedback on Filter Tabs and Refresh Icons

**Learning:** When using custom `<Icon>` components to replace native prop-based loading icons in `UiButton` elements, it's essential to implement the `group` class on the parent button and use `group-active` pseudo-classes on the child `<Icon>` to trigger CSS animations consistently across the entire interactive area.
**Action:** Use `active:scale-95` on standard interactive elements and `group-active:rotate-180` for custom tactile refresh animations to ensure responsive micro-interactions.
