...
## 2026-05-04 - Tactile Micro-Interactions on Generic Components
**Learning:** When applying custom tactile micro-interactions to generic components like `UiButton`, relying on built-in `loading` or `icon` props might mask or override the custom animation. The `group-active` classes on the parent combined with a direct child `<Icon>` give full control.
**Action:** Use a direct child `<Icon>` with `group-active:rotate-180` and conditionally apply `animate-spin` when pending to preserve custom touch animations.
