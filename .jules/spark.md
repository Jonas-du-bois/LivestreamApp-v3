...
## 2026-05-10 - Custom tactile micro-interactions on UiButton
**Learning:** When replacing a generic loading prop on a button with a custom animated icon, ensuring the animation is reactively bound to the component's loading state rather than only animating on touch events provides better feedback.
**Action:** Use `:class="{ 'animate-spin': pending }"` on the custom icon while removing `:loading` and keeping `:disabled="pending"` on the button.
