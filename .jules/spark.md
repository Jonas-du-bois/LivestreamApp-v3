...
## 2026-06-19 - Feedback tactile sur les cartes de Stream
**Learning:** Les cartes statiques de stream (bien que cliquables) manquent de feedback de pression physique, les rendant "plates" sur mobile. L'application d'effets `active:scale-[0.98]` combinés au changement de couleur d'arrière plan donne une sensation d'enfoncement satisfaisante.
**Action:** Toujours s'assurer que les composants cliquables en forme de carte intègrent un feedback sur l'état `active` (`scale` et variation `bg`), et propager cet effet aux éléments enfants pertinents via `group-active:` (comme l'icône de lecture et la miniature).
