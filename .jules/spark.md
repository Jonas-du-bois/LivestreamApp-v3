...
## 2024-05-01 - Animation du bouton Refresh
**Learning:** En remplaçant la prop statique `:loading` d'un bouton par un composant `<Icon>` explicite avec la classe `group-active:rotate-180` et une liaison conditionnelle `:class="{ 'animate-spin': pending }"`, on obtient un retour visuel tactile instantané au clic sans changement brusque d'icône.
**Action:** Toujours animer l'icône existante dans le `slot` du bouton plutôt que d'utiliser la prop générique `:loading` qui la remplace par un spinner. S'assurer de conserver `:disabled="pending"` pour bloquer les requêtes multiples et garantir le comportement correct du bouton.
