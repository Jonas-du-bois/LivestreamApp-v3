...
## 2026-05-17 - Feedback tactile sur les cartes statiques
**Learning:** Les cartes d'information statiques (comme l'organisation du weekend) paraissent "mortes" sous le doigt sur mobile si elles n'ont aucune réaction. L'ajout d'une très subtile interaction tactile (`active:scale-[0.98]`) sans en faire des boutons cliquables améliore le ressenti global de réactivité.
**Action:** Toujours ajouter un léger `active:scale-[0.98]` et `active:bg-white/5` sur les grosses cartes de contenu (`glass-card`), même si elles ne redirigent nulle part, pour simuler la pression du doigt sur l'écran.
