...
## 2026-06-05 - Custom Loading Animations in Generic Components
**Learning:** When applying custom tactile micro-interactions to generic components like `UiButton`, using their built-in `:loading` or `icon` props might mask or override the custom animation with a generic spinner.
**Action:** Instead of relying on the built-in props, insert a custom animated `<Icon>` directly into the component's default `<slot>` alongside the text, and use the Tailwind `group` class on the parent button with `group-active:rotate-180` on the child icon. Always ensure to explicitly add `:disabled="pending"` to prevent multiple submissions.
