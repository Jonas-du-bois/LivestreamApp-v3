...
## 2026-05-23 - Tactile Feedback on Segmented Controls

**Learning:** Navigation tabs and segmented controls without tactile feedback feel slow and unresponsive on mobile. Simply using `transition-all` and `hover` is not enough for touch interfaces.

**Action:** Always add pressure micro-interactions such as `active:scale-95`, `duration-200`, and `active:bg-white/5` to interactive navigation elements to ensure they respond immediately to finger presses, creating a premium feel.
