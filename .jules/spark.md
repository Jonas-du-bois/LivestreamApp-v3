...
## 2025-03-09 - Toggle Map Button
**Learning:** Adding complex micro-interactions (like rotate-scale icon swaps) using Vue's `<Transition>` can be done inline by passing Tailwind utility classes directly to the transition props (e.g., `enter-active-class="transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]"`). This keeps the component strictly utility-first without writing custom CSS in `<style scoped>`.
