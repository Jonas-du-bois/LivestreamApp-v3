...
## 2024-11-20 - Tactile Feedback on Glass Cards
- **Learning:** Long transition durations (e.g., 300ms) on basic active state interactions can make mobile interfaces feel sluggish. Deep scaling (e.g., `scale-[0.97]`) can sometimes feel too dramatic.
- **Action:** For interactive `UiGlassCard` components, prefer `transition-all duration-200` combined with a subtle `active:scale-[0.98]` to provide immediate, snappy, and satisfying tactile feedback without feeling exaggerated.
