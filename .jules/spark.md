...
## 2026-06-15 - Tactile Feedback Optimization
**Learning:** For optimal micro-interactions on touch devices, tactile feedback like `active:scale-[0.98]` combined with subtle color transitions makes UI components feel responsive. Be careful not to apply global animations to generic components (like `IconButton`) if the animation is only meant for a specific contextual action (like a refresh sync rotation), as this creates app-wide visual bugs.
**Action:** Use `active:scale-[0.98]` and `active:bg-white/5` for subtle color transitions and press feedback on GlassCards to simulate finger pressure.
