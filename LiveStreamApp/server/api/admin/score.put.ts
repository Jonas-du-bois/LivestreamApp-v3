...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // ⚡ Bolt: Calculate rank via DB index counting higher scores instead of O(N) memory scaling
    // We check explicitly if the current updated score is valid. If not, fallback to rank 0.
    const hasValidScore = typeof updated.score === 'number';
    let rank = 0;

    if (hasValidScore && updated.isPublished) {
