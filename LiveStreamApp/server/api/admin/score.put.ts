...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // ⚡ Bolt: Compute rank among published passages (per apparatus) using DB count instead of loading N docs into memory.
    // We only want published passages. Standard competition ranking assigns tied participants the same rank.
    let rank = 0;
    if (updated.isPublished && typeof updated.score === 'number') {
      const betterScoresCount = await PassageModel.countDocuments({
        isPublished: true,
