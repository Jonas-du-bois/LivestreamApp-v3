...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // BOLT: Optimize rank calculation memory footprint.
    // Use database-level countDocuments instead of loading full arrays into memory (O(N) bottleneck).
    // This naturally implements Standard Competition Ranking where tied scores share the same rank.
    let rank = 0;
