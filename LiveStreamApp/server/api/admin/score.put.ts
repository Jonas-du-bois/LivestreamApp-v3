...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // BOLT: Optimize memory usage and processing time by calculating standard competition rank
    // at the database level instead of fetching all documents and finding the index in memory.
    // This reduces an O(N) memory operation to O(1) in the application.
    let rank = 0;
