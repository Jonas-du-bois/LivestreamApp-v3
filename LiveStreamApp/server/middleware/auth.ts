...
export default defineEventHandler(async (event) => {
  ...
  // Security: Replace backslashes with forward slashes before normalization to prevent bypasses
  // via URL-encoded backslashes (e.g. %5c..%5c). Enforce a single leading slash.
  const sanitizedPath = '/' + path.replace(/\\/g, '/').replace(/^\/+/, '');
  const normalizedPath = posix.normalize(sanitizedPath);
