...
## 2024-05-24 - Interactive Refresh Icons\n\n**Learning:** Using generic `icon` and `loading` props on buttons prevents tactile micro-interactions on the icon itself. \n\n**Action:** Use an explicit child `<Icon>` with Tailwind `group` on the parent `UiButton`. Apply `transition-transform duration-200 group-active:rotate-180` for immediate tactile feedback, and bind `:class="{ 'animate-spin': pending }"` to seamlessly transition into a continuous loading state without relying on standard prop behaviors.
