...
\nJ'ai remarqué que les liens externes ('Liens utiles' sur infos.vue) avaient un état actif très basique (`active:scale-[0.98]`). Pour rendre l'interaction plus 'premium' et tactile, appliquer `active:scale-95` accompagné d'un changement de fond coloré (`active:bg-cyan-900/40`) et d'un liseré luminescent (`hover:shadow-[0_0_15px_rgba(...)]`) donne beaucoup plus de 'jus'. Le combo de translation d'icône d'ouverture sur l'état `group-active` (`group-active:translate-x-1 group-active:-translate-y-1`) renforce l'action de sortie de la page de manière très satisfaisante au doigt.
