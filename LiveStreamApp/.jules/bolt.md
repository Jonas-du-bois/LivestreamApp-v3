...
## 2024-05-20 - Rank Calculation Optimization
**Learning:** For calculating item ranks, fetching full collections into memory with `.find().sort()` and using `.findIndex()` causes O(N) memory bottlenecks and excessive CPU cycles, particularly as data grows.
**Action:** Always use database-level operations like `Model.countDocuments({ score: { $gt: targetScore } }) + 1` instead. This eliminates the memory bottleneck and naturally implements standard competition ranking (where tied participants share the same rank).
