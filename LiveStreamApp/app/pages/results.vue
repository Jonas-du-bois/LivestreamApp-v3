          ...
          <div v-if="(resultsResponse?.meta?.hasFinals || selectedRound === 'FINAL') && selectedDay?.toLowerCase() !== 'dimanche'" class="flex justify-center">
            <div class="glass-card p-1 rounded-full flex gap-1 border border-white/5 bg-white/5">
              <button
                ...
                class="px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-200 active:scale-95 active:bg-white/5"
          ...
          <template #actions>
            ...
            >
              <Icon
                ...
                :class="{ 'animate-spin': pending, 'active:rotate-180': !pending }"
              />
              {{ t('weather.refresh') }}
            </UiButton>
          </template>
