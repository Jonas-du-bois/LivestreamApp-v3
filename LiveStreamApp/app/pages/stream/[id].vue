      ...
      <div v-else-if="stream" key="stream-id-content" class="space-y-6">
        ...
        <div v-if="currentGroup">
          ...
          <div
            ...
            @keydown.enter="currentGroup._id && openGroupDetails(currentGroup._id, currentApparatus?.code)"
            @keydown.space.prevent="currentGroup._id && openGroupDetails(currentGroup._id, currentApparatus?.code)"
            role="button"
            tabindex="0"
            :aria-label="t('results.openGroupDetails', { group: currentGroup.name })"
            class="cursor-pointer active:scale-[0.98] transition-transform outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B1120] rounded-3xl"
          >
