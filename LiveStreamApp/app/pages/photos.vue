    ...
    <div v-else-if="showError" class="px-6">
      ...
      >
        <template #actions>
          <button
            type="button"
            class="app-focus-ring flex items-center gap-2 px-5 py-2.5 rounded-xl bg-cyan-500/15 border border-cyan-400/30 shadow-lg text-cyan-300 text-sm font-semibold transition-all duration-200 hover:bg-cyan-500/25 hover:shadow-cyan-500/10 active:scale-95 group"
            @click="refresh()"
          >
            <Icon name="fluent:arrow-sync-24-regular" class="w-4 h-4 transition-transform duration-500 group-active:rotate-180" :class="{ 'animate-spin': pending }" />
