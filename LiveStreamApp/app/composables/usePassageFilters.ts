...
// Cached formatter outside the loop/function for performance
const dayFormatter = new Intl.DateTimeFormat('fr-FR', { weekday: 'long', timeZone: 'Europe/Zurich' })
...
export const enrichPassage = (p: PassageEnriched): PassageSearchable => {
  ...
  let dayKey = ''
  if (p.startTime) {
    const ts = typeof p.startTime === 'string' ? Date.parse(p.startTime) : (p.startTime as any).getTime ? (p.startTime as any).getTime() : new Date(p.startTime).getTime()
    if (!Number.isNaN(ts)) {
