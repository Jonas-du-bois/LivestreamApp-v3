// Cache application-wide pour les formateurs Intl
...
function safeFormat(dateInput: string | number | Date, formatter: Intl.DateTimeFormat): string {
  const timestamp = typeof dateInput === 'string'
...
export const useTranslatedData = () => {
  ...
  const formatLocalizedDate = (dateInput: string | number | Date, options?: Intl.DateTimeFormatOptions): string => {
    ...
    return safeFormat(dateInput, formatter)
  }
